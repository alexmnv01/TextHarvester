...
- `out-error-list/` — папка с URL роликов, у которых не найден текст
...
- Если текст не найден, сохраняет URL в `out-list/parser-list-error-*.yaml` и создаёт отдельный `.txt` файл в `out-error-list/`.
...
- Для роликов без текстового описания создаются `.txt` файлы в `out-error-list/`; содержимое каждого файла — URL соответствующей страницы.
- Количество файлов в `out-error-list/` совпадает с количеством URL в `out-list/parser-list-error-*.yaml` за текущий запуск.
